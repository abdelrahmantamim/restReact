...
class App extends Component {
  ...
  fetchNearbyRestaurants = (latitude, longitude) => {
    ...
    headers.append('user-key', '4e18a7f5014b7232ff02d9c4e6e02f27');
    let options = {
      method: 'GET',
      headers: headers
    };
  ...
  createInfoWindow = (marker, popupContent) => {
    this.infoWindow = new window.google.maps.InfoWindow({maxWidth: 3000});
    let map = this.map;
    let that = this;
    ...
    return (
      <div className="App">
        <div className="App-header">
          ...
          <h2>Welcome to RackSpace resturant search</h2>
        </div>
        <div className="map-container">
          <p className="App-intro">
            <input className="form-control" id="autocomplete" placeholder="Enter a city" type="text" />
          </p>
          <div id="map"></div>
        </div>
      </div>
